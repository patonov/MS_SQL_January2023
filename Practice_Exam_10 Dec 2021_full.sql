...
CREATE or alter FUNCTION udf_FlightDestinationsByEmail(@email VARCHAR(70))
RETURNS INT
AS
BEGIN
	DECLARE @EmailToFind VARCHAR(70) = @Email
	DECLARE @CountToReturn INT
	SET @CountToReturn = 
	(SELECT COUNT(fd.PassengerId) FROM Passengers AS p JOIN FlightDestinations AS fd ON fd.PassengerId = p.Id
	GROUP BY p.Email
	HAVING p.Email = @email)

	IF @CountToReturn IS NULL
	BEGIN
	SET @CountToReturn = 0
	END
...
BEGIN
	...
	JOIN AircraftTypes AS aty ON aty.Id = ac.TypeId
